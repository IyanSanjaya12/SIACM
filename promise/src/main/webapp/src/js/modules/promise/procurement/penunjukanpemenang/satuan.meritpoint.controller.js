/**============================================================
 * Module: PenunjukanSatuanMeritPointController.js
 * Author: G.A.I
 ==============================================================*/
...
(function () {
    ...
    angular
        .module('naut')
        .controller('PenunjukanSatuanMeritPointController', PenunjukanSatuanMeritPointController);

    function PenunjukanSatuanMeritPointController($http, $scope, $rootScope, $resource, $location, $modal, toaster, $q, $timeout, ngTableParams) {
        var form = this;
            ...
            if (form.detailPengadaan.metodePenawaranHarga.id == 1) {
                $http.get($rootScope.backendAddress + '/procurement/evaluasiharga/EvaluasiHargaVendorDetailServices/getByPengadaanItem/' + $scope.pengadaanId + '/' + list.id).success(function (negosiasi, status, headers, config) {
                    angular.forEach(negosiasi, function (value, indeks) {
                        ...
                        $http.get($rootScope.backendAddress + '/procurement/penunjukanpemenangperitem/getByItemByVendorByPengadaan/' + list.id + '/' + value.evaluasiHargaVendor.vendor.id + '/' + $scope.pengadaanId).success(function (penetapan, status, headers, config) {
                ...
                $http.get($rootScope.backendAddress + '/procurement/evaluasiharga/EvaluasiHargaVendorDetailServices/getByPengadaanItem/' + $scope.pengadaanId + '/' + list.id).success(function (auction, status, headers, config) {
                    angular.forEach(auction, function (value, indeks) {
                        ...
                        $http.get($rootScope.backendAddress + '/procurement/penunjukanpemenangperitem/getByItemByVendorByPengadaan/' + list.id + '/' + value.evaluasiHargaVendor.vendor.id + '/' + $scope.pengadaanId).success(function (penunjukan, status, headers, config) {
                            if (penunjukan.length > 0) {
                                $rootScope.edit = true;
                                $rootScope.pemenang = penunjukan[0].id;
                                auction[indeks].status = true;
                                auction[indeks].nilaiKuantitas = penunjukan[0].nilaiKualitas;
                            }
                        })
                    })
                    var modalInstance = $modal.open({
        ...
        var modalInstanceController = function ($scope, $http, $modalInstance, $resource, $rootScope, data1, data2) {
            ...
            var simpanPemenang = function (indeks) {
                ...
                var uri = $rootScope.backendAddress + '/procurement/penunjukanpemenangperitem/';
            ...
            var getPenunjukanPemenangList = function (itemId, vendorId, pengadaanId) {
                $http.get($rootScope.backendAddress + '/procurement/penunjukanpemenangperitem/getByItemByVendorByPengadaan/' + itemId + '/' + vendorId + '/' + pengadaanId, {
                    ignoreLoadingBar: true
                }).success(function (data, status, headers, config) {
                    var id = 0;
                    if (data.length > 0) {
    ...
    PenunjukanSatuanMeritPointController.$inject = ['$http', '$scope', '$rootScope', '$resource', '$location', '$modal', 'toaster', '$q', '$timeout', 'ngTableParams'];
})();
