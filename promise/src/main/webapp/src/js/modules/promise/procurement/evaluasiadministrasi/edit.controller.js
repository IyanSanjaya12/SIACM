...
(function () {
    ...
    angular
        .module('naut')
        .controller('EvaluasiAdministrasiEditController', EvaluasiAdministrasiEditController);

    function EvaluasiAdministrasiEditController(ModalService, $http, $scope, $rootScope, $resource, $location, $modal, toaster, ngTableParams, $timeout, $q) {
        var eaec = this;
      ...
      /*  $scope.listVendor = function () {
        ...
        var ModalInstanceDetailCtrl = function ($scope, $rootScope, $modalInstance, $resource) {
            ...
            var eaecModal = this;
            if (typeof $rootScope.detilPengadaan !== 'undefined') {
                $scope.detailVendor = $rootScope.vendorEa.suratPenawaran;
            } else {
                console.log('INFO Error');
            }
            console.log('vendor ea = '+JSON.stringify($rootScope.vendorEa));
            eaec.pendaftaranVendorId = $rootScope.vendorEa.suratPenawaran.vendor.id;
                ...
                $http.get($rootScope.backendAddress + '/procurement/EvaluasiAdministrasiServices/getEvaluasiAdministrasiByVendorAndPengadaan/' + eaec.pendaftaranVendorId + '/' + eaec.pengadaanId).success(function (data, status, headers, config) {
                    if (data.length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            eaecModal.vendorId = $rootScope.vendorEa.suratPenawaran.vendor.id;
    ...
    EvaluasiAdministrasiEditController.$inject = ['ModalService', '$http', '$scope', '$rootScope', '$resource', '$location', '$modal', 'toaster', 'ngTableParams', '$timeout', '$q'];
