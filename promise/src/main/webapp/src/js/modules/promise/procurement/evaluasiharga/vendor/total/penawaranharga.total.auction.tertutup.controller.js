/**=========================================================
 * Module: PenawaranHargaTotalTertutupAddController.js
 * Author: F.H.K
 =========================================================*/
...
(function() {
    ...
    angular
        .module('naut')
        .controller('PenawaranHargaTotalTertutupAddController', PenawaranHargaTotalTertutupAddController);

    function PenawaranHargaTotalTertutupAddController($http, $rootScope, $resource, $location, $window, toaster, $scope, $filter, ngTableParams) {
        ...
        $http.get($rootScope.backendAddress + '/procurement/evaluasiharga/AuctionServices/getByPengadaan/' + $scope.pengadaanId, {
            ...
            angular.forEach(data,function(value,index){
                if(data[index].status != 1) {
                    ...
                    $rootScope.hargaAwalPenawaranPertama = data[index].hargaAwal;
        ...
        var historyDataPenawaran = function(auctionId) {
            ...
            $http.get($rootScope.backendAddress + '/procurement/evaluasiharga/PenawaranAuctionServices/getByAuction/' + auctionId, {
                ...
                angular.forEach(data,function(value,index) {
                    nilaiPenawaranList.push(data[index].nilaiPenawaran);
                });
                $scope.loading = false;
        ...
        var cariVendorInput = function(pesertaAuctionId, nilaiTerbaik1, nilaiTerbaik2, nilaiTerbaik3) {
            $http.get($rootScope.backendAddress + '/procurement/evaluasiharga/PenawaranAuctionServices/getByPesertaAuction/' + pesertaAuctionId, {
                ...
                //console.log("Hasil Penawaran Auction Per Vendor = " + JSON.stringify(data));
            	if(data.length > 0) {
            		$rootScope.hargaAwal = data[data.length - 1].nilaiPenawaran;
                    
                    $scope.penawaran = data[0].nilaiPenawaran;
                    ...
                    angular.forEach(data,function(value,index){
                        $scope.nilaiTerbaikList.push(data[index].nilaiPenawaran);
                    });
        ...
        $scope.mulaiAuction = function() {
            ...
            if($scope.hideTombol != 1) {
                if(tanggalInput > tanggalAkhirSesi) {
                    ...
                    $scope.go('/appvendor/promise/procurement/evaluasiharga/vendor/total/pemasukanpenawaran/tertutup');
                }
            } else {
    ...
    PenawaranHargaTotalTertutupAddController.$inject = ['$http', '$rootScope', '$resource', '$location', '$window', 'toaster', '$scope', '$filter', 'ngTableParams'];
