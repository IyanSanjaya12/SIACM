...
angular
    .module('naut')
    .controller('evendorDetailCtrl', ['$scope', 'RequestService', '$state', '$stateParams', '$modal', '$log', '$q','$rootScope', function ($scope, RequestService, $state, $stateParams, $modal, $log, $q, $rootScope) {

        $scope.pagingMaxBaris = 5;
        $scope.pagingTotalBaris = 0;
        $scope.pagingHalamanKe = 1;
        $scope.pagingMaxCatalog = 5;

        $scope.defaultImgVendor = './app/img/default_vendor.png';
        $scope.defaultImgBanner = './app/img/default_banner.jpg';
        $scope.defaultImgProduct = './app/img/no_image.png';
                ...
                .then(function (dataVP) {
                    ...
                    RequestService.doGET('/procurement/master/KontrakServices/getKontrakListByVendorId/' + id)
                        .then(function (data) {
                            ...
                            //vendor
                            if ($scope.vendorProfile.vendor.logoImage != null && typeof $scope.vendorProfile.vendor.logoImage !== 'undefined') {
        ...
        var catalogList = function (id) {
            var catalogVendor = {
                pageSize: $scope.pagingMaxCatalog,
