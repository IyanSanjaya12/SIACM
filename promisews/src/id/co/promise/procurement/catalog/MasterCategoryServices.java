...
@Path(value = "/procurement/catalog/CategoryServices")
@Produces(MediaType.APPLICATION_JSON)
public class MasterCategoryServices {
	...
	@Path("/getCategoryByNama/{nama}")
	@GET
	public Category getHariLiburByNama(@PathParam("nama") String nama) {
		return categorySession.getCategoryByNameInd(nama);
	}
	...
	public Map insert(Category category,
		...
		Boolean isSaveTranslateId = categorySession.checkTranslateId(category.getTranslateInd(), toDo, category.getId());
	  
		if(!isSaveTranslateId) {
			String errorTranslateId = "Nama Kategori Tidak Boleh Sama";
			Boolean isValid = false;
			map.put("isValid", isValid);
			map.put("errorTranslateId", errorTranslateId);
		}
	 
		if(isSaveTranslateId) {
			categorySession.insertCategory(category, tokenSession.findByToken(token));
		}
	...
	public Map update(Category category,
		...
		if(isSaveTranslateId) {
			categorySession.updateCategory(category, tokenSession.findByToken(token));
		}
	...
	@Path("/delete")
	@POST
	public Category deleteCategory(@FormParam("id") int id,
			@HeaderParam("Authorization") String token) {
		return categorySession.deleteCategory(id, tokenSession.findByToken(token));
	}
	/*@Consumes(MediaType.APPLICATION_JSON)
