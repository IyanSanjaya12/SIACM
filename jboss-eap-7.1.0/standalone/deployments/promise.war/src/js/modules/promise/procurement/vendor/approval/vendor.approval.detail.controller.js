...
(function () {
	...
	angular
		.module('naut')
		.controller('VendorApprovalDetailController', VendorApprovalDetailController);

	function VendorApprovalDetailController(RequestService, $http, $scope, $rootScope, $filter, ngTableParams, $modal, $state, FileUploader, toaster, $location, $stateParams, ModalService) {
...
//				ModalService.closeModalInformation();
		...
		function fillForm()
		{
            ...
            form.DeskripsiPerusahaan    = String(form.vendorBean.deskripsi).replace(/<[^>]+>/gm, '')
		...
		uploadFileApproval.filters.push({
			...
			fn: function (item /*{File|FileLikeObject}*/ , options) {
				return item.type=='application/vnd.openxmlformats-officedocument.wordprocessingml.document' || item.type=='application/msword'
                    ...
                    || item.type=='application/pdf' || item.type=='application/vnd.ms-excel';
			}
		});
	...
	VendorApprovalDetailController.$inject = ['RequestService', '$http', '$scope', '$rootScope', '$filter', 'ngTableParams', '$modal', '$state', 'FileUploader', 'toaster','$location', '$stateParams', 'ModalService'];
