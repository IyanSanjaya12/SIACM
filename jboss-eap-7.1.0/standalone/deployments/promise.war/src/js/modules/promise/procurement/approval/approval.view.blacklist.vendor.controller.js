 ...
 * Modul/Tahapan ID: 8
 * Author: Reinhard
 * blacklistvendors.approval.details.controller
 =========================================================*/
(function () {
    'use strict';
    ...
    angular
        .module('naut')
        .controller('ApprovalViewBlacklistVendorController', ApprovalViewBlacklistVendorController);

    function ApprovalViewBlacklistVendorController(RequestService, $http, $scope, $rootScope, $filter, ngTableParams, $modal, $state, toaster, $location, $stateParams, ModalService, $timeout) {
        ...
        $scope.simpan = function (statusId) {
            ...
            RequestService.doPOST('/procurement/vendormanagement/BlacklistVendorServices/updateBlacklistApproval', formPost)
        ...
        RequestService.doGET('/procurement/vendormanagement/BlacklistVendorServices/getBlacklistVendorMaxVendor/' + form.approvalProcess.approvalProcessType.valueId)
            .then(function successCallback(data) {
                form.blacklistVendorDtl = data;
  	             RequestService.doGET('/procurement/vendormanagement/UnblacklistVendorServices/getUnblacklistVendorMaxAndBlacklistId/' + form.approvalProcess.approvalProcessType.valueId + '/' + form.blacklistVendorDtl.blacklistVendor.blacklistVendorId)
                    .then(function successCallback(data2) {

                        if (data2 != null && data2 != undefined && data2 != '') {
                            console.info(data2)
                            form.canUnblacklist = false;
                            form.unblacklistVendorDtl = data2;
                        }

                        console.info(form.canUnblacklist)

                        console.info(form.unblacklist)
                    ...
                    .success(function (data, status, headers, config) {
                        ...
                        $http.get($rootScope.backendAddress + '/procurement/master/jabatan/get-list')
                            .success(function (data, status, headers, config) {
                                ...
                                $http.get($rootScope.backendAddress + '/procurement/master/kualifikasiVendorServices/getKualifikasiVendorList')
                                    .success(function (data, status, headers, config) {
    ...
    ApprovalViewBlacklistVendorController.$inject = ['RequestService', '$http', '$scope', '$rootScope', '$filter', 'ngTableParams', '$modal', '$state', 'toaster', '$location', '$stateParams', 'ModalService', '$timeout'];
