/**=========================================================
 * Module: negosiasi.vendor.satuan.detail.controller.js
 * Modul/Tahapan ID: 15
 * Author: Reinhard
 =========================================================*/
...
(function() {
    ...
    angular
        .module('naut')
        .controller('NegosiasiVendorSatuanDetailController', NegosiasiVendorSatuanDetailController);
    function NegosiasiVendorSatuanDetailController($scope, $http, $rootScope, $resource, $location, toaster, $filter, $stateParams, editableOptions, editableThemes,$anchorScroll) {
    	
    	
    	/*BYPASS LOGIN*/
    	
    	/*$rootScope.userRoleName = 'VENDOR';
    	$rootScope.userRoleName = {"user":{"id":2,"username":"vendor01","password":"fcea920f7412b5da7be0cf42b8c93759","email":"vendor01@mmi-pt.com"},"token":"82ec47d4c1f144960915a21cac2482b4"};
   */
        ...
        function loadVendorNegosiasi() { 
        	...
        	$http.get($rootScope.backendAddress + '/procurement/vendor/vendorServices/getVendorByUserId/' + $rootScope.userRoleName.user.id, {
            	...
            	$http.get($rootScope.backendAddress + '/procurement/pemasukanpenawaran/suratPenawaranServices/getListSuratPenawaranByVendorAndPengadaan/' + negosiasiDetail.vendor.id +'/'+$scope.pengadaanId , {
                	...
                	$http.get($rootScope.backendAddress + '/procurement/negosiasi/NegosiasiServices/getLastFromPanitiaByPengadaanAndVendor/' + $scope.pengadaanId+'/'+negosiasiDetail.vendor.id , {
                    	...
                    	/*default value*/
                    	negosiasiDetail.negosiasiLastFromPanitia = {"nilaiPenawaranAfterCondition":0};
      ...
      $scope.showItemCP = function(cp) {
        ...
        if(cp.conditionalPrice) {
          selected = $filter('filter')($scope.masterItemCP, {id: cp.conditionalPrice.id});
        }
      ...
      $scope.showItemPengadaan = function(cp) {
          var selected = [];
          if(cp.itemPengadaan) {
            selected = $filter('filter')($scope.itemPengadaanList, {id: cp.itemPengadaan.id});
          }
      ...
      $scope.editItem = function(rowform){
        ...
        $scope.savingIntoNegosiasi = function (lastOffer)
        {
        	...
        	var postData ={
    				...
    				"userId" 	 :$rootScope.userRoleName.user.id,
    				"isVendor" 	 :1,
    				"isPanitia"	 :0,
    ...
    NegosiasiVendorSatuanDetailController.$inject = ['$scope', '$http', '$rootScope', '$resource', '$location', 'toaster', '$filter','$stateParams','editableOptions', 'editableThemes','$anchorScroll'];
