...
(function () {
    ...
    angular
        .module('naut')
        .controller('NegosiasiTotalDetailCetakController', NegosiasiTotalDetailCetakController);

    function NegosiasiTotalDetailCetakController(ModalService, $scope, $http, $rootScope, $resource, $location, toaster, $filter, $stateParams) {
                    ...
                    angular.forEach(negosiasiDetail.vendorNegosiasiList, function (vendorNegosiasi, vendorNegosiasiIndex) {
                        ...
                        $http.get($rootScope.backendAddress + '/procurement/negosiasi/NegosiasiServices/getFirstFromVendorByPengadaanAndVendor/' + $scope.pengadaanId + '/' + vendorNegosiasi.vendor.id, {
                            ...
                            if (data.length > 0) {
                                ...
                                $http.get($rootScope.backendAddress + '/procurement/pembukaanPenawaran/KursPengadaanServices/getKursPengadaanByPengadaanAndMataUang/' + $scope.pengadaanId + '/' + vendorNegosiasi.negosiasiFirstFromVendor.mataUang.id)
                                    .success(function (data, status, headers, config) {
        ...
        $scope.printDiv = function (divName) {
            ...
            if (!$rootScope.promiseDirectiveViewDataPengadaan.isMaterialNotEmpty) {
            	$(".barang-panel").hide();
            } else if (!$rootScope.promiseDirectiveViewDataPengadaan.isJasaNotEmpty) {
            	$(".jasa-panel").hide();
            }
            
            if (!$scope.updateLastOfferStatus) {
            	$(".last-offer").hide();
            } else if ($scope.updateLastOfferStatus) {
            	$(".dealing").hide();
            }
    ...
    NegosiasiTotalDetailCetakController.$inject = ['ModalService', '$scope', '$http', '$rootScope', '$resource', '$location', 'toaster', '$filter', '$stateParams'];
