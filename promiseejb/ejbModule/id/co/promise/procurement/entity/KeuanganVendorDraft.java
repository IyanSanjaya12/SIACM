...
@Entity
@Table(name = "T3_KEUANGAN_V_DRAFT")
@NamedQueries({
	@NamedQuery(name="KeuanganVendorDraft.findByVendor",
			query="SELECT keuanganVendorDraft FROM KeuanganVendorDraft keuanganVendorDraft WHERE keuanganVendorDraft.vendor.id =:vendorId AND keuanganVendorDraft.isDelete=0"),
    @NamedQuery(name="KeuanganVendorDraft.findByVendorAndKeuanganVendor",
    		query="SELECT keuanganVendorDraft FROM KeuanganVendorDraft keuanganVendorDraft WHERE keuanganVendorDraft.vendor.id =:vendorId AND keuanganVendorDraft.keuanganVendor.id=:keuanganVendorId AND keuanganVendorDraft.isDelete = 0"),
    @NamedQuery(name ="KeuanganVendorDraft.deleteKeuanganVendorDraft", 
    		query ="UPDATE KeuanganVendorDraft keuanganVendorDraft set keuanganVendorDraft.isDelete = 1 WHERE keuanganVendorDraft.vendor.id =:vendorId ")
})
@TableGenerator(name = "tableSequence", table = "SEQUENCE", pkColumnName = "TABLE_SEQ_NAME", 
valueColumnName = "TABLE_SEQ_VALUE", pkColumnValue = "T3_KEUANGAN_V_DRAFT", initialValue = 1, allocationSize = 1)
public class KeuanganVendorDraft {
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@Column(name = "KEUANGAN_VENDOR_DRAFT_ID")
	private Integer id;

	@OneToOne
	@JoinColumn(name = "KEUANGAN_VENDOR_ID", referencedColumnName = "KEUANGAN_VENDOR_ID")
