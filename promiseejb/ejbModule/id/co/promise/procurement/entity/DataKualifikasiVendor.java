...
@Entity
@NamedQueries({
		@NamedQuery(name = "DataKualifikasiVendor.find", query = "select x from DataKualifikasiVendor x where x.isDelete=0"),
		@NamedQuery(name = "DataKualifikasiVendor.findByPengadaanAndVendor", query = "select x from DataKualifikasiVendor x where x.isDelete=0 and x.pengadaan.id=:pengadaanId and x.vendor.id=:vendorId") })
@Table(name = "T5_DATA_KUALIFIKASI_V")

@TableGenerator(name = "tableSequence", table = "SEQUENCE", pkColumnName = "TABLE_SEQ_NAME", 
valueColumnName = "TABLE_SEQ_VALUE", pkColumnValue = "T5_DATA_KUALIFIKASI_V", initialValue = 1, allocationSize = 1)
public class DataKualifikasiVendor {

	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	@Column(name = "DATA_KUALIFIKASI_VENDOR_ID")
	private Integer id;
	...
	@Column(name = "TANGGAL_KEUANGAN_UPDATE")
	private Date tanggalKeuanganUpdate;
	...
	public Date getTanggalKeuanganUpdate() {
		return tanggalKeuanganUpdate;
	}

	public void setTanggalKeuanganUpdate(Date tanggalKeuanganUpdate) {
		this.tanggalKeuanganUpdate = tanggalKeuanganUpdate;
	}
